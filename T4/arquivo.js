...
var myserver = http.createServer(function(req,res) {
    ...
    if(req.method == 'GET'){
        if((args[1] == 'musica') && (!isNaN(args[2]))){
            fs.readFile('data/doc' + args[2] + '.xml', (erro, dados)=>{
                if(!erro){
                    ...
                    res.write('Erro na leitura do ficheiro ' + 'data/doc' + args[2] + '.xml...')
                }
                res.end()  
            });      
        }
        else if(args[2] == 'doc2html.xsl'){
        ...
        else if(req.url == '/music.png'){
            fs.readFile('music.png', (erro, dados)=>{
                if(!erro){
                    res.writeHead(200, {'Content-Type':'image/png'}) 
                    res.write(dados);
                }
                else {
                    console.log('erro na musica')
                    res.writeHead(200, {'Content-Type':'text/plain'}) 
                    res.write('Erro na leitura do CSS...')
                }
                res.end()  
